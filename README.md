...
它会悬浮在屏幕最顶层,实时显示 Codex 的工作状态:**派活后坐下用 MacBook 工作,需要输入时抬头等你,干完活进入 Ready 检查状态,翻车了瘫倒在地**。

![MacBook 工作](previews/running.gif) ![科目三](previews/waving.gif) ![失败](previews/failed.gif)

## 特色

在标准哆啦A梦宠物包的基础上定制了两个动画:

- **MacBook 工作**:Codex 在后台执行任务时,哆啦A梦坐在小笔记本前专注工作(原版是原地小跑)。坐姿头身比与站立帧严格对齐,切换时不会突然变大
- **官方配色校准**:全表主体蓝校准到官方哆啦A梦标准蓝 `#00A0E9`,各状态配色统一
- **科目三打招呼**:保留在 waving 行,用于宠物打招呼/首次唤醒一类轻量反馈。按官方契约 waving 行为 4 帧,从 8 帧原始动作中抽取关键帧制作
...
| 状态 | 预览 | 触发时机 |
|---|---|---|
| 发呆 idle | ![idle](previews/idle.gif) | 没有任务时,站立呼吸眨眼 |
| MacBook 工作 running | ![running](previews/running.gif) | Codex 在后台干活时 |
| 检查 review | ![review](previews/review.gif) | 任务完成后有未读结果/Ready 时 |
| 等待 waiting | ![waiting](previews/waiting.gif) | 等你批准操作(气泡显示红色时钟) |
| 跳跃 jumping | ![jumping](previews/jumping.gif) | 鼠标悬停宠物时的临时反馈 |
| 科目三 waving | ![waving](previews/waving.gif) | 宠物打招呼/首次唤醒一类轻量反馈 |
...
- MacBook 工作和科目三两行动画为 AI 生成后,经色键抠图、按头宽与基线对齐缩放、碎片清理后合成
